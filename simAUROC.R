...
lowMeta <- data.frame(p=c(0.5, 0.3, 0.15, 0.06, 0.1, 0.15, 0.3, 0.45),
...
# chance level metacog participant
chanceMeta <- data.frame(p=c(0.255,0.244,0.245,0.2560,0.260,0.245,0.240,0.255),
                      conf=c(1,2,3,4,1,2,3,4),
                      correct=c(1,1,1,1,0,0,0,0))

chanceMetaAUC <- data.frame(pcor = c(0,cumsum(rev(chanceMeta[chanceMeta$correct==1,]$p))),
                         pinc = c(0,cumsum(rev(chanceMeta[chanceMeta$correct==0,]$p))))
...
lowMeta %>%
  ...
  geom_point(aes(x=1.5,y=0.52),shape=15,cex=2)+
  geom_point(aes(x=2.5,y=0.52),shape=16,cex=2)+
  geom_point(aes(x=3.5,y=0.52),shape=17,cex=2)+
...
lowMetaAUC %>%
  ...
  geom_line()+
  geom_point(aes(x=pinc[2],y=pcor[2]),shape=17,cex=2)+
  geom_point(aes(x=pinc[3],y=pcor[3]),shape=16,cex=2)+
  geom_point(aes(x=pinc[4],y=pcor[4]),shape=15,cex=2)+
...
highMeta %>%
  ...
  theme_classic(base_size=12)+
  scale_fill_manual(values=c('black','darkgray'))+
  theme(legend.position='none',aspect.ratio=1) -> e
